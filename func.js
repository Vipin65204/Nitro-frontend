...
populate = () => {
  ...
  $.each(sourceData.uniqueArrayObj, function (index, dt) {
    // Create a new list item element with a span inside it
    const listItem = $('<li><span class="caret"> ' + dt + '</span></li>')
    ...
    //  ------ post by sort ------
    const filterPost = sourceData.data.filter(
      (item) => item[sourceData.sortBy[sourceData.checkboxSelection]] === dt
    )
    filterPost.forEach((postsByWeek) => {
      timeStampItem =
        postsByWeek[sourceData.viewBy[sourceData.checkboxSelection]]
      ...
      liElement.click(function () {
        ...
        $('.profile-image').attr(
          'src',
          `https://picsum.photos/id/${globalVariable}/50/50`
        )
...
const fetchData = async (url) => {
  try {
    ...
    // // Adding the "weekNumber" and "year" properties to each item in the array
...
saveEdit = () => {
  $('#saveChangesBtn').click(function () {
    ...
    // Check if the location and author is valid string
    if (isValidLocation && isValidAuthor) {
      // Performs the save operation or other logic
     

      const editedPostIndex = sourceData.data.findIndex(
        (post) => post.id === globalVariable
      )
